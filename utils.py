...
def get_main_keyboard(user_id, user_ui_lang):
    keyboard = [[InlineKeyboardButton(get_text(user_id, 'robot_dev', user_ui_lang), callback_data='custom_btn')],
...
def get_dev_keyboard(user_id, user_ui_lang):
    keyboard = [[InlineKeyboardButton(get_text(user_id, 'dev_types', user_ui_lang), callback_data='dev_types')],
...
def get_type_keyboard(user_id, user_ui_lang):
    funcs = [("双向联系", 'contact'), ("宫水AI", 'gsai'), ("设置", 'setting'), ("群管", 'group_manage'), ("查询", 'query'), ("资源", 'resource'), ("签到", 'checkin'), ("AI", 'ai_sub')]
    keyboard = []
